...
static DWORD WINAPI threadOperation(LPVOID arg) {
	...
	while (ReadFile(operationFile, &operation, sizeof(OPERATION), &readOp, NULL) && readOp == sizeof(OPERATION)) {
		...
		fileReserved.QuadPart = sizeof(OPERATION);
		ov.Offset = filePos.LowPart;
		ov.OffsetHigh = filePos.HighPart;
		ov.hEvent = 0;

#ifdef MY_DEBUG
		ReportInfo(_T("Waiting lock for id: %d\n"), operation.id);
#endif

		// Lock account file
		if (!LockFileEx(*(param->accountFile), LOCKFILE_EXCLUSIVE_LOCK, 0, fileReserved.LowPart, fileReserved.HighPart, &ov)) {
			ReportError(_T("LockFileEx"));
			CloseHandle(operationFile);
			ExitThread(3);
		}

#ifdef MY_DEBUG
		ReportInfo(_T("Acquired lock for id: %d\n"), operation.id);
#endif

		// Read account
		if (!ReadFile(*(param->accountFile), &account, sizeof(ACCOUNT), &readAcc, &ov) || readAcc != sizeof(ACCOUNT)) {
			ReportError(_T("ReadFile"));
			UnlockFileEx(*(param->accountFile), 0, fileReserved.LowPart, fileReserved.HighPart, &ov);
			CloseHandle(operationFile);
			ExitThread(4);
		}
		...
		// Unlock account file
		if (!UnlockFileEx(*(param->accountFile), 0, fileReserved.LowPart, fileReserved.HighPart, &ov)) {
			ReportError(_T("UnlockFileEx"));
			CloseHandle(operationFile);
			ExitThread(6);
		}

#ifdef MY_DEBUG
		ReportInfo(_T("Released lock for id: %d\n"), operation.id);
#endif
	}
