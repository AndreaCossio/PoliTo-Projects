...
typedef struct {
	...
	LPHANDLE accountFile, semaphore;
} TARG, * LPTARG;
...
INT _tmain(INT argc, LPTSTR argv[]) {
	...
	// Create semaphore
	if ((semaphore = CreateSemaphore(NULL, 1, 1, NULL)) == NULL) {
	...
	for (i = 0; i < nThreads; i++) {
		...
		threadArguments[i].semaphore = &semaphore;
...
#ifdef MY_DEBUG
		ReportInfo(_T("Waiting semaphore\n"), operation.id);
#endif

		WaitForSingleObject(*(param->semaphore), INFINITE);

#ifdef MY_DEBUG
		ReportInfo(_T("Entered semaphore\n"), operation.id);
#endif
...
#ifdef MY_DEBUG
		ReportInfo(_T("Released semaphore\n"), operation.id);
#endif
	}
