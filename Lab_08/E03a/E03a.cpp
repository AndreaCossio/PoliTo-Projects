...
BOOL readStudent(HANDLE file, DWORD n) {
	...
	// Set file position
	if (SetFilePointerEx(file, offset, &pointer, FILE_BEGIN) == 0) {
		_ftprintf(stderr, _T("Cannot SetFilePointerEx(). Error: %x\n"), GetLastError());
		return FALSE;
	}

	// Read student information from file
	if (!ReadFile(file, &s, sizeof(student_t), &read, NULL)) {
...
BOOL writeStudent(HANDLE file, DWORD n) {
	...
	if (!WriteFile(file, &s, sizeof(student_t), &written, NULL)) {
